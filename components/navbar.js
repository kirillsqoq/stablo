...
export default function Navbar(props) {
  ...
  const rightmenu = [
    ...
    {
      label: "Shop",
      href: "https://onlyfame.mysellix.io",
      external: true,
      // badge: "new"
    },
    // {
                  ...
                  {leftmenu.map((item, index) => (
                    <Fragment key={`${item.label}${index}`}>
                      {item.children && item.children.length > 0 ? (
                        ...
                        >
                          <span className="text-lg font-bold hover:scale-110 hover:underline transition-transform duration-300">
                            {item.label}
                          </span>
                        </Link>
                      )}
